...
func (receiver BrokerActionCoinDcx) ExecuteMarketOrder(order common.RequestMarketOrder) (common.Order, error) {
	...
	res := `{"orders":[{"id":"ead19992-43fd-11e8-b027-bb815bcb14ed","market":"%s","order_type":"%s","side":"%s","status":"open","fee_amount":0.000008,"fee":0.1,"total_quantity":%f,"remaining_quantity":%f,"avg_price":0,"price_per_unit":0.00001567,"created_at":"%s","updated_at":"%s"}]}`
	res = fmt.Sprintf(res, order.Market, string(order.OrderType), string(order.ActionType), order.Quantity, order.Quantity, time.Now().Format(time.RFC3339), time.Now().Format(time.RFC3339))
